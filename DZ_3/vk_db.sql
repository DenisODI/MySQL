...
CREATE TABLE IF NOT EXISTS media_likes (
  ...
  media_id INT NOT NULL COMMENT "Ссылка на медиафайл, которому поставили лайк",
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT "Время создания строки",
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT "Время обновления строки"
...
CREATE TABLE IF NOT EXISTS messages_likes (
  id INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT "Идентификатор строки",
  from_user_id INT UNSIGNED NOT NULL COMMENT "Ссылка на пользователя, который поставил лайк",
  messages_id INT NOT NULL COMMENT "Ссылка на сообщение, которому поставили  лайк",
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT "Время создания строки",
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT "Время обновления строки"
...
-- Таблица лайков (объединяющая три предудыщих)
CREATE TABLE IF NOT EXISTS likes (
  -- id INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT "Идентификатор строки", -- Если не использовать составной ключ
  from_user_id INT UNSIGNED NOT NULL COMMENT "Ссылка на пользователя, который поставил лайк",
  object_id INT UNSIGNED NOT NULL COMMENT "Ссылка на объект, которому поставили лайк",
  object_type VARCHAR(255) NOT NULL COMMENT "Тип события, которому постаивли лайк",
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT "Время создания строки",
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT "Время обновления строки",
